...
export const RatingSection = ({ locationId }: { 
    ...
    const fetchRatings = async () => {
        try {
            setLoading(true)
            const response = await axios.get(`http://localhost:8000/ratings/?location_id=${locationId}`)
    ...
    return(
        <Box>
            ...
            {ratings.map((rating) => (
                <Box key={rating.id}>
                    <RatingStars readOnly size="small" value={rating.rating}></RatingStars>
            ...
            <Box>
                ...
                <RatingStars name="half-read" value={ratingValue} onChange={(e, newValue) => setRatingValue(newValue || 0)} />
                <TextField
